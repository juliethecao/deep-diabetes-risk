...
from plotting import plot_training_curves, plot_roc_and_pr, plot_confusion_matrix, plot_calibration, plot_latent_tsne, plot_feature_ablation
...
class FocalLoss(nn.Module):
    # weigh is difficulty of classification
    # can be used to compare of not, up to us
    def __init__(self):
        super().__init__()
    ...
    def forward(self, logits, targets):
        ...
        # care more about getting the healthy patients right
        alpha = targets * 0.25 + (1 - targets) * 0.75  
        # care more about getting the diabete patients right
        #alpha_t = targets * 0.25 + (1 - targets) * 0.75  
        gamma = 2.0 # the "focusing" strength --> higher mean only care more on the correctness and punish more on incorrectness
...
def main():
    ...
    # we want to punish more for missing a diabetic person
    loss_func = choose_loss_func("weighted_bce", pos_weight) #can be changed to another type
    ...
    for epoch in range(1, 30):
        ...
        val = evaluate(model, val_loader, loss_func, device)[0]  # [0] = metrics dict
        scheduler.step()
        ...
        if val["roc_auc"] > best_val_auc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), "results_deep_mlp/best_model.pt")
        else:
            patience_counter += 1
    ...
    with open("results_deep_mlp/test_metrics.json", "w") as f:
        json.dump(te, f, indent=2)

    print("Generating plots...")
    plot_training_curves(train_losses, val_losses, val_metrics_history, "results_deep_mlp")
    plot_roc_and_pr(test_labels, test_probs, "results_deep_mlp")
    plot_confusion_matrix(test_labels, test_preds, "results_deep_mlp")
    plot_calibration(test_labels, test_probs, "results_deep_mlp")
    plot_latent_tsne(model, test_loader, device, "results_deep_mlp")

    print("Running feature ablation (may take a minute)...")
    drops = plot_feature_ablation(model, X_test, y_test, FEATURE_NAMES, device, "results_deep_mlp")
    with open("results_deep_mlp/feature_ablation.json", "w") as f:
        json.dump(drops, f, indent=2)

    print("\nAll done! Check the 'results_deep_mlp/' folder for all outputs.")
